...
def get_days_to_expiry():
    ...
    initial_expiry = datetime.datetime.today() + relativedelta(days=8)
...
def trail_target_and_exit(kite, exchange='MCX', trail_buffer=10, sl_gap=500, minimum_profit_cap=5000):
    """
...
def add_sl_and_target_on_fno_positions(kite, stop_loss=-7500, exchange='NFO', trail_trigger=5000, trail_gap=1000):
    now = datetime.datetime.now().time()
    cutoff = datetime.time(23, 30) if exchange == 'MCX' else datetime.time(16, 30)
...
def get_expected_positions_by_premium(kite, premium_targets=None, range_limit=20):
    if premium_targets is None:
        res = calculate_daily_from_vix(kite)
        days = get_days_to_expiry()
        g1 = res.get('vix', 1) * days * 0.75
        premium_targets = decrease_by_20_percent_fixed(g1, 4)
